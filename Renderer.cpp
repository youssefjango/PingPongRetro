...
const float LETTER_SPACING = 12.5f;
internal void draw_text(const char *txt, float x, float y, float size_x, float size_y, u32 color) {
	...
	while (*txt) {
		if (*txt != 32) {
			const char** letter = letters[*txt - '0'];
			float original_x = x;
			for (int i = 0; i < 5; i++) {
